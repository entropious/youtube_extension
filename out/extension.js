...
let proxyPort = 0;
function getProxyEmbedHtml(videoId) {
    return `<!DOCTYPE html>
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(YouTubeViewProvider.viewType, provider));
    context.subscriptions.push(vscode.commands.registerCommand('youtube-panel.loadUrl', async () => {
...
class YouTubeViewProvider {
    ...
    _getHistory() {
        const raw = this._state.get(YouTubeViewProvider.historyKey, []);
        return raw
            .map((item) => {
...
YouTubeViewProvider.viewType = 'youtube-panel.view';
YouTubeViewProvider.historyKey = 'youtube-history';
